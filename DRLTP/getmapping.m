...
if strcmp(mappingtype,'nru2') %Uniform 2
    x = zeros(1,2^samples);
    unique_x = zeros(1,2^samples);
    for i = 0:2^samples-1
        x(i+1) = bitshift(i,4,samples);
        for j = 0:3
            x(i+1) = bitset(x(i+1),j+1,bitget(i,samples-3+j));
        end
        if (i==x(i+1))
            unique_x(i+1) = 1;
        end
    ...
    index = 0;
    for i = 0:length(y)-1
        j = bitset(bitshift(y(i+1),1,samples),1,bitget(y(i+1),samples)); %rotate left
        numt = sum(bitget(bitxor(y(i+1),j),1:samples)); %number of 1->0 and
        %0->1 transitions
        %in binary string
        %x is equal to the
        %number of 1-bits in
...
if strcmp(mappingtype,'nrexu2') %Uniform 2
    x = zeros(1,2^samples);
    unique_x = zeros(1,2^samples);
    for i = 0:2^samples-1
        x(i+1) = bitshift(i,4,samples);
...
if strcmp(mappingtype,'nrlbpu2') %Uniform 2
    x = zeros(1,2^samples);
    for i = 0:2^samples-1
        x(i+1) = bitcmp(i,samples);
        x(i+1) = min(i,x(i+1));
    end
    [y,~,n] = unique(x,'first');
    index = 0;
